...
import { z } from 'zod'
import { findCountryTimezone } from './country-timezones.js'
import { generateImage } from './generate-image.js'
import { formatGeocodedLocation, searchLocations } from './geocode.js'
import {
    fetchCurrentWeather,
...
server.registerTool(
    ...
    async ({ query, count }) => {
        ...
        if (locations === undefined) {
            const errorText = '위치 정보를 가져오지 못했습니다. 잠시 후 다시 시도해 주세요.'
            return {
                isError: true,
                content: [
                    {
...
server.registerTool(
    ...
    async ({ latitude, longitude }) => {
        ...
        if (!weather) {
            const errorText = '날씨 정보를 가져오지 못했습니다. 잠시 후 다시 시도해 주세요.'
            return {
                isError: true,
                content: [
                    {
...
server.registerTool(
    ...
    async ({ prompt, num_inference_steps }) => {
        const result = await generateImage(prompt, num_inference_steps)
        ...
        const serverInfo = {
            ...
            tools: [
                ...
                },
                {
                    name: 'global_time',
                    description:
                        '나라 이름을 입력하면 해당 나라의 현재 시간과 날씨를 반환합니다.'
                },
                {
                    name: 'geocode',
                    description:
                        '도시명, 지명, 우편번호로 위도·경도 좌표를 조회합니다.'
                },
                {
                    name: 'get_weather',
                    description: '위도·경도 좌표로 현재 날씨를 조회합니다.'
                },
                {
                    name: 'generate-image',
                    description:
            ...
            prompts: [
                {
                    name: 'code_review',
                    description:
                        '코드를 입력받아 언어와 무관하게 단계적으로 상세 리뷰합니다.'
                }
            ],
            limits: {
